    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var marker = await UserManager.GeneratePasswordResetTokenAsync(user.Id);
                // создаем ссылку для подтверждения
                var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = user.Id, Marker = marker },
                           protocol: Request.Url.Scheme);
                // отправка письма

                await UserManager.SendEmailAsync(user.Id, "Сброс пароля",
                           "Сбросьте ваш пароль, щелкнув:: <a href=\""
                                                           + callbackUrl + "\">здесь</a>");
        ...
        {
            return Marker == null ? View("Error") : View();
        }
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (user == null)
            {
                ...
                var marker = model.Marker.Replace(" ", "+");

                var result = await UserManager.ResetPasswordAsync(user.Id, marker, model.Password);
            if (result.Succeeded)
            {
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
